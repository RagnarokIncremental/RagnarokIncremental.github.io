...
/*vot*/			$expirydate = dgmdate($member['groupterms']['main']['time'], 'Y-m-d');
...
/*vot*/					'<input type="text" class="txt" size="9" name="extgroupexpirynew['.$group['groupid'].']" value="'.(in_array($group['groupid'], $extgrouparray) && !empty($member['groupterms']['ext'][$group['groupid']]) ? dgmdate($member['groupterms']['ext'][$group['groupid']], 'Y-m-d') : '').'" onclick="showcalendar(event, this)" />'
				), TRUE);
			}
...
/*vot*/		$member['regdate'] = dgmdate($member['regdate'], 'Y-m-d H:i:s');
/*vot*/		$member['lastvisit'] = dgmdate($member['lastvisit'], 'Y-m-d H:i:s');
